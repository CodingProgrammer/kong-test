...
if [ ! -d "${DOCKER_DIR}" ]; then
    echo "⚠️  Warning: docker directory does not exist, skipping cleanup"
    exit 0
fi
...
if [ ! -f "docker-compose.yml" ]; then
    echo "⚠️  Warning: docker-compose.yml does not exist, skipping cleanup"
    exit 0
fi
...
if [ ! -z "$CONTAINER_IDS" ]; then
    echo "⚠️  Found remaining containers, cleaning up..."
