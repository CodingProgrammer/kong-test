...
if (!fs.existsSync(DOCKER_DIR)) {
    console.log('⚠️  docker directory does not exist, skipping cleanup');
    process.exit(0);
}

// Check if docker-compose.yml exists
if (!fs.existsSync(DOCKER_COMPOSE_FILE)) {
    console.log('⚠️  docker-compose.yml does not exist, skipping cleanup');
    process.exit(0);
}
...
try {
    ...
    console.error('⚠️  Warning: Failed to stop containers');
}
...
try {
    ...
    if (containers.trim()) {
        console.log('⚠️  Found remaining containers, cleaning up...');
